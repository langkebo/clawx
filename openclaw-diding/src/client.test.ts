...
import { clearTokenCache, isTokenCached, getTokenCacheInfo, clearClientCache } from "./client.js";
...
describe("Feature: dingtalk-integration, Property 5: Token 缓存行为", () => {
  ...
  it("should clear token cache when clearTokenCache is called", () => {
    const clientIdsArb = fc.array(fc.string({ minLength: 1, maxLength: 50 }), {
      minLength: 1,
      maxLength: 10,
    });
  ...
  it("should only clear specified clientId when clearTokenCache is called with argument", () => {
    const clientIdPairArb = fc
      .tuple(fc.string({ minLength: 1, maxLength: 50 }), fc.string({ minLength: 1, maxLength: 50 }))
