...
- [Viking] Router: add LRU cache for routing decisions (5-minute TTL, max 1000 entries) to reduce repeated routing API calls for similar prompts.
- [Viking] Router: implement exponential backoff failover with rate-limit handling for routing model calls.
- [Viking] Router: add `classifyProviderError()` for structured error classification (rate_limit, auth, format, billing, transient, unknown).
- [Viking] Router: add 6 new provider environment mappings (minimax, zhipu, deepseek, kimi, yi, baichuan).
- [Viking] Tasks: add background task registry for async routing operations with retry support.
- [Viking] Search: add SearXNG self-hosted search provider integration for privacy-focused web search.
- Security/Exec: add Unicode spoofing protection with zero-width character detection and homoglyph normalization.
- Security/Exec: extend forbidden environment variables list with 20+ additional Git and SSL-related variables.
- Security/Device: add device token scope validation with hierarchy expansion and expiration checks.
- Security/WebSocket: add pre-authentication guards with frame size limits, rate limiting, and handshake timeout.
- Types: replace `any` types with proper interfaces in discord/sender-identity, auto-reply/reply, session-tool-result-guard, and session-memory handler.
