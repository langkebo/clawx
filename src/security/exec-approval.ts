...
const INVISIBLE_FORMATTING = /[\u2000-\u200A\u2028\u2029\u00AD\u034F\u061C\u17B4\u17B5\u180E\u2065\uFE00-\uFE0F]/gu;
...
export function escapeInvisibleUnicode(text: string): string {
  return text
    ...
    .replace(INVISIBLE_FORMATTING, (char) => `\\u{${char.charCodeAt(0).toString(16).padStart(4, '0')}}`)
...
export function isCommandObfuscated(command: string): boolean {
  ...
  const homoglyphCount = Object.keys(DANGEROUS_HOMOGLYPHS).filter(h => command.includes(h)).length;
